...
- qa flavor 추가 후 flutter pub run flutter_flavorizr 를 실행. issue 발생.
- Xcode -> Runner -> Configurations -> Debug, Release, Profile을 수정(Pods-Runner.debug, Pods-Runner.release, Pods-Runner.profile)
- cd ios && pod install 
...
```shell
❯ keytool -exportcert -alias release -keystore ~/.android/release.jks -storepass android -keypass android | openssl sha1 -binary | openssl base64
